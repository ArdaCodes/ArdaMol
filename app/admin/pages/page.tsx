...
const FIELDS: { key: string; label: string; group: string; type?: "input" | "textarea" }[] = [
  // Hero
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-500">
            Edit every piece of text shown across the site.
          </p>
        </div>
        <button
          form="pages-form"
          className="rounded-md bg-stone-900 px-5 py-2.5 text-sm text-white"
        >
      ...
      <form id="pages-form" onSubmit={handleSave} className="mt-8 max-w-2xl space-y-10 pb-16">
        {GROUPS.map((group) => (
          <div key={group}>
            <h2 className="mb-4 border-b border-stone-200 pb-2 font-serif text-lg text-stone-900">
              {group}
            </h2>
